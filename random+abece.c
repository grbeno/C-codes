...
#define ELEM 5
#define MAX 10 //leghosszabb elõforduló név
...
int main() {
	...
	char **nevsor = (char**)malloc(sizeof(char*)*ELEM);
	for (i = 0; i < ELEM; i++) {
		nevsor[i] = (char*)malloc(MAX); //másoláskor változóak a méretek, 2.megoldás: realloc() a rendezõ függvényben!
	...
	for (j = 0; j < ELEM; j++) {
		for (k = j + 1; k < ELEM; k++) {
			if (strcmp(nevsor[j], nevsor[k]) > 0) { //magyar ábécére nem jó!
				temp = (char*)malloc(strlen(nevsor[j])+1);
